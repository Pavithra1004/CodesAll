...
public class FetchRanking {
	WebDriver driver;
	@Test
	public void fetchRanking() throws SQLException

	{
		...
		{
			  actualData = teams.get(i).getText();
		}
		...
		try {
			...
			while(result.next())
			{
				...
				for(int i=0;i<=teams.size()-1;i++)
				{
					int result1=statement.executeUpdate("insert into IPL_Ranking values('"+(j+1)+"','"+teams.get(i).getText()+"','"+Mcount.get(i).getText()+"','"+Wount.get(i).getText()+"','"+Lcount.get(i).getText()+"');");
					j++;
					if(result1>=1)
					{
					...
					while(result2.next())
					{
					String excpectedData = result2.getString("TeamName");
					SoftAssert ast=new SoftAssert();
					ast.assertEquals(actualData, excpectedData);
